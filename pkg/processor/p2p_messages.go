...
// P2pMessage represents a pending P2P message
type P2pMessage struct {
	Status         events.P2pMessageStatus
	SentTime       interface{} // stores the sent event
	ReceivedTime   interface{} // stores the received event
	ConfirmedEvent interface{} // stores the final confirmed universal event
}

// P2pMessageProcessor collects and matches send/receive P2P message pairs
type P2pMessageProcessor struct {
	ctx context.Context

	// Maps for each message type using their respective keys
	...
	// Confirmed universal events
	confirmedEvents []interface{}
}

// NewP2pMessageProcessor returns a P2pMessageProcessor
func NewP2pMessageProcessor(ctx context.Context) *P2pMessageProcessor {
	return &P2pMessageProcessor{
		ctx:                  ctx,
		voteMessages:         make(map[string]*P2pMessage),
		blockPartMessages:    make(map[string]*P2pMessage),
		proposalMessages:     make(map[string]*P2pMessage),
		proposalPOLMessages:  make(map[string]*P2pMessage),
		newRoundStepMessages: make(map[string]*P2pMessage),
		hasVoteMessages:      make(map[string]*P2pMessage),
		voteSetMaj23Messages: make(map[string]*P2pMessage),
		voteSetBitsMessages:  make(map[string]*P2pMessage),
		confirmedEvents:      make([]interface{}, 0),
	}
}

// Process implements EventProcessor
func (p *P2pMessageProcessor) Process(evt events.Event) error {
	switch e := evt.(type) {
...
func (p *P2pMessageProcessor) processSendVote(e *events.EventSendVote) error {
	...
	if existing := p.voteMessages[key]; existing != nil && existing.Status == events.P2pMsgStatusReceived {
		...
		confirmedEvent := &events.EventP2pVote{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeP2pVote,
				Timestamp:        recv.Timestamp,
				NodeId:           recv.NodeId,
				ValidatorAddress: recv.ValidatorAddress,
			},
			Vote: recv.Vote,
			P2pInfo: events.P2pInfo{
				Status:          events.P2pMsgStatusConfirmed,
				SenderPeerId:    e.NodeId,
				RecipientPeerId: e.RecipientPeerId,
				SentTime:        e.Timestamp,
				ReceivedTime:    recv.Timestamp,
				Latency:         recv.Timestamp.Sub(e.Timestamp),
			},
		}
		existing.Status = events.P2pMsgStatusConfirmed
		existing.SentTime = e
		existing.ConfirmedEvent = confirmedEvent
		p.confirmedEvents = append(p.confirmedEvents, confirmedEvent)
...
func (p *P2pMessageProcessor) processReceiveVote(e *events.EventReceivePacketVote) error {
	...
	if msg == nil {
		// Received without corresponding send - create receive-only entry
		msg = &P2pMessage{
			Status:       events.P2pMsgStatusReceived,
			ReceivedTime: e,
		}
	} else {
		// Found matching send - create confirmed universal event
		sendEvent := msg.SentTime.(*events.EventSendVote)
		confirmedEvent := &events.EventP2pVote{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeP2pVote,
			...
			P2pInfo: events.P2pInfo{
				Status:          events.P2pMsgStatusConfirmed,
				SenderPeerId:    sendEvent.NodeId,
				RecipientPeerId: sendEvent.RecipientPeerId,
				SentTime:        sendEvent.Timestamp,
				ReceivedTime:    e.Timestamp,
				Latency:         e.Timestamp.Sub(sendEvent.Timestamp),
			},
		}
		msg.Status = events.P2pMsgStatusConfirmed
		msg.ReceivedTime = e
		msg.ConfirmedEvent = confirmedEvent
		p.confirmedEvents = append(p.confirmedEvents, confirmedEvent)
	}
...
// Block part message processing
func (p *P2pMessageProcessor) processSendBlockPart(e *events.EventSendBlockPart) error {
	...
	if existing := p.blockPartMessages[key]; existing != nil && existing.Status == events.P2pMsgStatusReceived {
		...
		confirmedEvent := &events.EventP2pBlockPart{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeP2pBlockPart,
				Timestamp:        recv.Timestamp,
				NodeId:           recv.NodeId,
				ValidatorAddress: recv.ValidatorAddress,
			},
...
func (p *P2pMessageProcessor) processReceiveBlockPart(e *events.EventReceivePacketBlockPart) error {
	partHash := hex.EncodeToString(e.Part.Hash())
	...
	if msg == nil {
		...
		sendEvent := msg.SentTime.(*events.EventSendBlockPart)
		confirmedEvent := &events.EventP2pBlockPart{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeP2pBlockPart,
				Timestamp:        e.Timestamp,
...
func (p *P2pMessageProcessor) processSendProposal(e *events.EventSendProposal) error {
	...
	if existing := p.proposalMessages[key]; existing != nil && existing.Status == events.P2pMsgStatusReceived {
		...
		confirmedEvent := &events.EventP2pProposal{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeP2pProposal,
				Timestamp:        recv.Timestamp,
				NodeId:           recv.NodeId,
				ValidatorAddress: recv.ValidatorAddress,
			},
...
func (p *P2pMessageProcessor) processReceiveProposal(e *events.EventReceivePacketProposal) error {
	...
	if msg == nil {
		...
		sendEvent := msg.SentTime.(*events.EventSendProposal)
		confirmedEvent := &events.EventP2pProposal{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeP2pProposal,
				Timestamp:        e.Timestamp,
...
// ProposalPOL message processing
func (p *P2pMessageProcessor) processSendProposalPOL(e *events.EventSendProposalPOL) error {
	...
	if existing := p.proposalPOLMessages[key]; existing != nil && existing.Status == events.P2pMsgStatusReceived {
		...
		confirmedEvent := &events.EventP2pProposalPOL{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeP2pProposalPOL,
				Timestamp:        recv.Timestamp,
				NodeId:           recv.NodeId,
				ValidatorAddress: recv.ValidatorAddress,
			},
			Height:           recv.Height,
			ProposalPolRound: recv.ProposalPolRound,
			ProposalPol:      recv.ProposalPol,
			P2pInfo: events.P2pInfo{
				Status:          events.P2pMsgStatusConfirmed,
				SenderPeerId:    e.NodeId,
				RecipientPeerId: e.RecipientPeerId,
...
func (p *P2pMessageProcessor) processReceiveProposalPOL(e *events.EventReceivePacketProposalPOL) error {
	...
	if msg == nil {
		...
		sendEvent := msg.SentTime.(*events.EventSendProposalPOL)
		confirmedEvent := &events.EventP2pProposalPOL{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeP2pProposalPOL,
				Timestamp:        e.Timestamp,
			...
			Height:           e.Height,
			ProposalPolRound: e.ProposalPolRound,
			ProposalPol:      e.ProposalPol,
			P2pInfo: events.P2pInfo{
				Status:          events.P2pMsgStatusConfirmed,
				SenderPeerId:    sendEvent.NodeId,
				RecipientPeerId: sendEvent.RecipientPeerId,
...
// NewRoundStep message processing
func (p *P2pMessageProcessor) processSendNewRoundStep(e *events.EventSendNewRoundStep) error {
	...
	if existing := p.newRoundStepMessages[key]; existing != nil && existing.Status == events.P2pMsgStatusReceived {
		...
		confirmedEvent := &events.EventP2pNewRoundStep{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeP2pNewRoundStep,
				Timestamp:        recv.Timestamp,
				NodeId:           recv.NodeId,
				ValidatorAddress: recv.ValidatorAddress,
			},
			...
			Round:                 e.Round,
			Step:                  e.Step,
			SecondsSinceStartTime: int32(recv.SecondsSinceStartTime),
			LastCommitRound:       recv.LastCommitRound,
			P2pInfo: events.P2pInfo{
				Status:          events.P2pMsgStatusConfirmed,
				SenderPeerId:    e.NodeId,
				RecipientPeerId: e.RecipientPeerId,
...
func (p *P2pMessageProcessor) processReceiveNewRoundStep(e *events.EventReceivePacketNewRoundStep) error {
	...
	if msg == nil {
		...
		sendEvent := msg.SentTime.(*events.EventSendNewRoundStep)
		confirmedEvent := &events.EventP2pNewRoundStep{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeP2pNewRoundStep,
				Timestamp:        e.Timestamp,
			...
			Round:                 e.Round,
			Step:                  e.Step,
			SecondsSinceStartTime: int32(e.SecondsSinceStartTime),
			LastCommitRound:       e.LastCommitRound,
			P2pInfo: events.P2pInfo{
				Status:          events.P2pMsgStatusConfirmed,
				SenderPeerId:    sendEvent.NodeId,
				RecipientPeerId: sendEvent.RecipientPeerId,
...
// HasVote message processing
func (p *P2pMessageProcessor) processSendHasVote(e *events.EventSendHasVote) error {
	...
	if existing := p.hasVoteMessages[key]; existing != nil && existing.Status == events.P2pMsgStatusReceived {
		...
		confirmedEvent := &events.EventP2pHasVote{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeP2pHasVote,
				Timestamp:        recv.Timestamp,
				NodeId:           recv.NodeId,
				ValidatorAddress: recv.ValidatorAddress,
			},
...
func (p *P2pMessageProcessor) processReceiveHasVote(e *events.EventReceivePacketHasVote) error {
	...
	if msg == nil {
		...
		sendEvent := msg.SentTime.(*events.EventSendHasVote)
		confirmedEvent := &events.EventP2pHasVote{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeP2pHasVote,
				Timestamp:        e.Timestamp,
...
// VoteSetMaj23 message processing
func (p *P2pMessageProcessor) processSendVoteSetMaj23(e *events.EventSendVoteSetMaj23) error {
	...
	if existing := p.voteSetMaj23Messages[key]; existing != nil && existing.Status == events.P2pMsgStatusReceived {
		...
		confirmedEvent := &events.EventP2pVoteSetMaj23{
			BaseEvent: events.BaseEvent{
...
func (p *P2pMessageProcessor) processReceiveVoteSetMaj23(e *events.EventReceivePacketVoteSetMaj23) error {
	...
	if msg == nil {
		...
		sendEvent := msg.SentTime.(*events.EventSendVoteSetMaj23)
		confirmedEvent := &events.EventP2pVoteSetMaj23{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeSendVoteSetMaj23,
				Timestamp:        e.Timestamp,
...
// VoteSetBits message processing
func (p *P2pMessageProcessor) processSendVoteSetBits(e *events.EventSendVoteSetBits) error {
	...
	if existing := p.voteSetBitsMessages[key]; existing != nil && existing.Status == events.P2pMsgStatusReceived {
		...
		confirmedEvent := &events.EventP2pVoteSetBits{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeP2pVoteSetBits,
				Timestamp:        recv.Timestamp,
				NodeId:           recv.NodeId,
				ValidatorAddress: recv.ValidatorAddress,
			},
...
func (p *P2pMessageProcessor) processReceiveVoteSetBits(e *events.EventReceivePacketVoteSetBits) error {
	...
	if msg == nil {
		...
		sendEvent := msg.SentTime.(*events.EventSendVoteSetBits)
		confirmedEvent := &events.EventP2pVoteSetBits{
			BaseEvent: events.BaseEvent{
				EventType:        events.EventTypeP2pVoteSetBits,
				Timestamp:        e.Timestamp,
...
// Helper functions
// Build robust, direction-aware keys including necessary dimensions
func (p *P2pMessageProcessor) getVoteKey(height uint64, round uint64, voteType string, validatorIndex uint64, sender, receiver string) string {
	return fmt.Sprintf("vote:%d:%d:%s:%d:%s:%s", height, round, voteType, validatorIndex, sender, receiver)
}

func (p *P2pMessageProcessor) getBlockPartKey(height int64, round int32, partHashHex, sender, receiver string) string {
	return fmt.Sprintf("blockpart:%d:%d:%s:%s:%s", height, round, partHashHex, sender, receiver)
}

func (p *P2pMessageProcessor) getProposalKey(height int64, round int32, blockHash, sender, receiver string) string {
	return fmt.Sprintf("proposal:%d:%d:%s:%s:%s", height, round, blockHash, sender, receiver)
}

func (p *P2pMessageProcessor) getProposalPOLKey(height int64, polRound int32, sender, receiver string) string {
	return fmt.Sprintf("proposalpol:%d:%d:%s:%s", height, polRound, sender, receiver)
}

func (p *P2pMessageProcessor) getNewRoundStepKey(height int64, round int32, step, sender, receiver string) string {
	return fmt.Sprintf("newroundstep:%d:%d:%s:%s:%s", height, round, step, sender, receiver)
}

func (p *P2pMessageProcessor) getHasVoteKey(height int64, round int32, voteType string, index int32, sender, receiver string) string {
	return fmt.Sprintf("hasvote:%d:%d:%s:%d:%s:%s", height, round, voteType, index, sender, receiver)
}

func (p *P2pMessageProcessor) getVoteSetMaj23Key(height int64, round int32, voteType, blockHash, sender, receiver string) string {
	return fmt.Sprintf("votesetmaj23:%d:%d:%s:%s:%s:%s", height, round, voteType, blockHash, sender, receiver)
}

func (p *P2pMessageProcessor) getVoteSetBitsKey(height int64, round int32, voteType, blockHash, sender, receiver string) string {
...
func (p *P2pMessageProcessor) GetResults() ([]interface{}, string) {
	return p.confirmedEvents, "p2p_messages"
}
