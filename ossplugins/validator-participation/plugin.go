...
func (p *Plugin) Name() string { return "validator-participation" }
func (p *Plugin) Init(ctx pluginsdk.Context) error {
	p.ctx = ctx
	p.ep = NewValidatorParticipationProcessor(ctx.Ctx)
	return nil
}
...
func init() {
	pluginloader.Register("validator-participation", func() pluginsdk.Plugin { return &Plugin{} })
}
